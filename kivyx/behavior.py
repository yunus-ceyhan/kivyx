...
from kivy.graphics import Color, Ellipse, StencilPush, StencilPop, \
	StencilUse, StencilUnUse, RoundedRectangle
from kivyx.theming import Theming


class CommonRipple(Theming):
...
class RectangularBehavior(ButtonBehavior,CommonRipple):
	ripple_scale = NumericProperty(2.75)
	ripple_radius = ListProperty([0,])

	def lay_canvas_instructions(self):
		with self.canvas.after:
			StencilPush()
			RoundedRectangle(pos=self.pos, size=self.size, radius=self.ripple_radius)
			StencilUse()
	...
	def _set_ellipse(self, instance, value):
		super(RectangularBehavior, self)._set_ellipse(instance, value)
		self.ellipse.pos = (self.ripple_pos[0] - self.ripple_rad / 2.,
		                    self.ripple_pos[1] - self.ripple_rad / 2.)


class CircularBehavior(ButtonBehavior,CommonRipple):
	...
	def _set_ellipse(self, instance, value):
		super(CircularBehavior, self)._set_ellipse(instance, value)
